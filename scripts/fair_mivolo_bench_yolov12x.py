...
parser = argparse.ArgumentParser(description='Fair MIVOLO Tracking Benchmark with YOLOv12x')
...
parser.add_argument('--output-dir', type=str, default='../results/bench_YOLOv12x',
...
class YOLOv12PersonDetector:
    def __init__(
        ...
        # Find person class ID in COCO classes
        self.person_class_id = None
        for class_id, class_name in self.detector_names.items():
            if class_name.lower() == 'person':
                self.person_class_id = class_id
                break
        
        if self.person_class_id is None:
            raise ValueError("Person class not found in model. Make sure you're using a COCO-trained model.")
    ...
    def predict(self, image: Union[np.ndarray, str]) -> Results:
        ...
        if len(results.boxes) > 0:
            # Keep only person class detections
            person_mask = results.boxes.cls == self.person_class_id
            if person_mask.any():
...
# Configuration - Updated for YOLOv12x
YOLO_WEIGHTS   = pathlib.Path('../weights/yolo12x.pt')  # Changed to YOLOv12x
...
if not YOLO_WEIGHTS.exists():
    ...
    print("Please download YOLOv12x weights and place them in the weights folder")
    exit(1)
...
try:
    ...
    test_settings = {
        "skip_frames": 5,
        "conf_threshold": 0.25,  # Lower threshold for outdoor CCTV
        "iou_threshold": 0.5,
        "max_frames": None,
        "filter_faces": False,  # Not needed for YOLOv12x
...
FILTER_FACES = test_settings.get('filter_faces', False)  # Not used for YOLOv12x
...
print(f"\n=== YOLOv12x Tracking Benchmark Configuration ===")
print(f"YOLO Model: YOLOv12x (COCO classes)")
...
print(f"Detection: Person class only (COCO dataset)")
...
# Initialize YOLOv12x detector
print(f"\n=== Initializing YOLOv12x Detector ===")
try:
    detector = YOLOv12PersonDetector(
    ...
    print(f"✅ YOLOv12x Detector initialized successfully on {device}")
except Exception as e:
...
for test_num, tracker_key in enumerate(tracker_order, 1):
    ...
    while cap.isOpened():
        ...
        if frames % SKIP_FRAMES == 0:
            ...
            # Get detections using YOLOv12x detector
            try:
                results = detector.predict(frame)  # No filter_faces parameter needed
                
                if len(results.boxes) > 0:
                    # Extract detection data (persons only from COCO classes)
        ...
        # Draw tracks on frame (persons only)
        active_detections = 0
        for track in tracks:
            if len(track) >= 7:
                x1, y1, x2, y2, track_id, conf, cls = track[:7]
    ...
    result = {
        ...
        'filter_faces': False,  # Not applicable for YOLOv12x
...
print(f"✅ YOLOv12x COCO model used for person detection")
print(f"✅ Multiple parameter configurations tested")
print(f"✅ Better performance expected on outdoor CCTV footage")
...
print(f"\n🎯 Optimized for outdoor CCTV with YOLOv12x COCO model")
